...
Weak FTP credentials allow attackers to easily compromise FTP accounts by exploiting common or weak passwords. This can lead to unauthorized access to sensitive files and potential further exploitation of the system.

### **Vulnerability Explanation:** Initial Access 
##### #2. Uploading a PHP Reverse Shell to an FTP Server to an Accessible Directory on a Web Server

Uploading a PHP reverse shell to an FTP server and storing it in an accessible directory on a web server allows attackers to execute arbitrary commands remotely. If the PHP file is placed in a publicly accessible directory, an attacker can trigger the reverse shell, gaining full control over the web server and potentially escalating the attack to other network systems.

### **Vulnerability Fix:** Initial Access

##### #1. Weak FTP Credentials

To mitigate this vulnerability, enforce strong password policies by requiring complex and unique credentials for FTP accounts. Implement multi-factor authentication (MFA) where possible and restrict FTP access to authorized users only. Additionally, regularly audit FTP logs for suspicious login attempts and configure FTP to use secure protocols such as FTPS or SFTP for encrypted data transfer.
...
To mitigate this vulnerability, ensure that file uploads to FTP servers are restricted to authorized and validated file types. Implement file scanning to detect malicious scripts such as reverse shells. Additionally, disable execution permissions on directories that store user-uploaded files and use proper access control mechanisms. Regularly monitor file integrity and audit web server logs for suspicious activities.
...
Readable configuration files can expose sensitive information, such as database credentials, API keys, and system settings, to unauthorized users. If these files are accessible, attackers may exploit this information to gain unauthorized access, escalate privileges, or compromise the integrity and confidentiality of the application and its data.

### **Vulnerability Explanation:** Privilege Escalation

##### #2.MySQL User-Defined Functions - Root Privileges

MySQL User-Defined Functions (UDFs) can be exploited if an attacker gains the ability to create or modify UDFs with root privileges. This allows them to execute arbitrary code as the root user, potentially compromising the entire system by performing unauthorized operations, altering data, or escalating privileges.

### **Vulnerability Fix:** Privilege Escalation

##### #1. Readable Config File

To mitigate this risk, restrict access to configuration files by setting file permissions to limit read access only to necessary users or processes. Store sensitive configurations outside the web root directory, encrypt sensitive values within config files, and ensure access controls and logging are in place to monitor unauthorized access attempts.
...
To mitigate this risk, restrict permissions for UDF creation and ensure only trusted users can define them. Additionally, configure MySQL to run with limited privileges and avoid granting the FILE privilege to untrusted users, which can prevent loading malicious UDF libraries.
